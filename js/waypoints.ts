 ...
 */
export function setStartWaypoint(location: google.maps.LatLng, displayName: string, id: string | null = null): void {
 ...
 */
export function setDestinationWaypoint(location: google.maps.LatLng, displayName: string, id: string | null = null): void {
    const newWaypoint: Waypoint = {
        type: 'destination',
        name: displayName,
        location,
...
export function handleAddStationToRoute(stationId: string | number): void {
    ...
    callbacks.showTemporaryMessage?.(callbacks.translate?.('messageStationAlreadyInRoute') ?? 'messageStationAlreadyInRoute', false);
        return;
    }
    ...
    callbacks.showTemporaryMessage?.(callbacks.translate?.('messageStationAddedToRoute', { name: waypoint.name }) ?? 'messageStationAddedToRoute', false);
...
export function handleWaypointAction(event: MouseEvent): void {
    ...
    if (button.classList.contains('remove-waypoint-btn')) {
        ...
        [state.routeWaypoints[index], state.routeWaypoints[index - 1]] = [state.routeWaypoints[index - 1], state.routeWaypoints[index]];
    } else if (button.classList.contains('move-waypoint-down-btn') && index < state.routeWaypoints.length - 1) {
        [state.routeWaypoints[index], state.routeWaypoints[index + 1]] = [state.routeWaypoints[index + 1], state.routeWaypoints[index]];
    }
...
export function clearAllWaypoints(): void {
    ...
    callbacks.showTemporaryMessage?.(callbacks.translate?.('messageRoutePlanCleared') ?? 'messageRoutePlanCleared', false);
}
