...
export class App {
    ...
    private setupCallbacks(): void {
        // UI callbacks
        setupUICallbacks({
            handleBrandPreferenceChange: (brand, action) => this.handleBrandPreferenceChange(brand, action),
            handleUnignoreStationClick: (stationId) => this.handleUnignoreStationClick(stationId),
     ...
     */
    private async initializeMapAndServices(center: { lat: number; lng: number }, zoom: number): Promise<void> {
    ...
    private initializeLocationService(): void {
        ...
        const onLocationSet = ({ type, location, displayName, markerTitle }: {
            type: 'start' | 'end';
    ...
    private initializeAutocomplete(): void {
        try {
            ...
            if (startInput && endInput && this.locationService) {
                this.mapComponent.initializeAutocomplete(
                    startInput,
                    endInput,
                    (type) => this.locationService!.onPlaceChanged(type)
                );
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (this.stationDataComponent.allStationData.length > 0) {
                    showTemporaryMessage(translate('messageLoadedStations', {
    ...
    private setupEventListeners(): void {
        // Directions changed
        this.mapComponent.directionsRenderer?.addListener('directions_changed', () => this.handleDirectionsChanged());
        ...
        // Paste buttons
        pasteStartBtn?.addEventListener('click', () => this.locationService?.handlePasteButtonClick('start'));
        pasteEndBtn?.addEventListener('click', () => this.locationService?.handlePasteButtonClick('end'));
        startInput?.addEventListener('paste', (e) => this.locationService?.handleDirectPaste(e as ClipboardEvent, 'start'));
        endInput?.addEventListener('paste', (e) => this.locationService?.handleDirectPaste(e as ClipboardEvent, 'end'));

        // Use location buttons
        useLocationStartBtn?.addEventListener('click', () => this.locationService?.handleUseCurrentLocationClick('start'));
        useLocationEndBtn?.addEventListener('click', () => this.locationService?.handleUseCurrentLocationClick('end'));
        ...
        // Filter inputs
        filterInputs.connectorType.forEach(input => input.addEventListener('change', () => this.handleFilterChange()));
        filterInputs.powerLevel.forEach(input => input.addEventListener('change', () => this.handleFilterChange()));
        filterInputs.serviceType.forEach(input => input.addEventListener('change', () => this.handleFilterChange()));
        ...
        // Offset controls
        offsetStartIncBtn?.addEventListener('click', () => this.handleOffsetChange('start', OFFSET_INCREMENT_KM));
        offsetStartDecBtn?.addEventListener('click', () => this.handleOffsetChange('start', -OFFSET_INCREMENT_KM));
        offsetDestIncBtn?.addEventListener('click', () => this.handleOffsetChange('dest', OFFSET_INCREMENT_KM));
        offsetDestDecBtn?.addEventListener('click', () => this.handleOffsetChange('dest', -OFFSET_INCREMENT_KM));
    ...
    calculateRoute(useOptimization: boolean = false): void {
        ...
        console.log(`Calculating route with ${state.routeWaypoints.length} waypoints. Optimization: ${useOptimization}`);
    ...
    handleOpenRouteInGmaps(): void {
        ...
        const locationsString = allPoints.map(wp => {
    ...
    applyFilters(): number {
        ...
        this.markerComponent.visiblePoiMarkers.forEach(marker => {
            ...
            if (favBadge) {
                const isFav = marker.poiData?.brand && this.preferencesComponent.favoriteBrands.has(marker.poiData.brand);
    ...
    resetAllFilters(): void {
        ...
        const defaultMapType = (google.maps.MapTypeId as any)[DEFAULT_MAP_TYPE_ID.toUpperCase()] || google.maps.MapTypeId.ROADMAP;
    ...
    toggleBrandFilterMode(): void {
        ...
        const messageKey = this.preferencesComponent.brandFilterMode === 'favoritesOnly'
            ? 'messageShowOnlyFavs'
    ...
    triggerDetourPrecalculation(): void {
        ...
        this.detourComponent.triggerPrecalculation(
            updateMarkerDetourText,
            fetchPoiDetails
        );
    }
    ...
    async calculateAndDisplayDetourOnClick(
        ...
        const result = await this.detourComponent.calculateDetourForPoi(
            marker, poiLocation, startLoc, endLoc, originalDist, originalDur
        );
     ...
     */
    private handleClearClick(inputElement: HTMLInputElement, clearButtonElement: HTMLElement, locationType: LocationType): void {
    ...
    saveSettings(): void {
        ...
        setStorageItem(IGNORED_STATIONS_STORAGE_KEY, Array.from(this.preferencesComponent.ignoredStationIds));
    ...
    loadSettings(): void {
        ...
        if (generalSettings) {
            if (generalSettings.filters) {
                loadedFilters.connectorType = (generalSettings.filters.connectorType as typeof defaultFilters.connectorType) || defaultFilters.connectorType;
            ...
            loadedDistanceThreshold = (typeof dist === 'number' && dist >= 1 && dist <= 20) ? dist : DEFAULT_DISTANCE_THRESHOLD;
            loadedFilters.mapTypeId = (generalSettings.mapTypeId as typeof DEFAULT_MAP_TYPE_ID) || DEFAULT_MAP_TYPE_ID;
        }
...
window.retryDetourCalculation = (stationId: string) => {
    ...
    if (marker && state.isRouteActive && state.startLocation && state.endLocation &&
        state.originalRouteDistance !== null && state.originalRouteDuration !== null && marker.position) {
        const poiPos = toLatLng(marker.position);
        getApp().calculateAndDisplayDetourOnClick(
            marker, poiPos, state.startLocation, state.endLocation,
            state.originalRouteDistance, state.originalRouteDuration
        );
    }
};
