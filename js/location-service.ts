import { SHORT_URL_RESOLVER_PROXY_TEMPLATE } from './config.js';
import { isPlaceInTurkey } from './utils.js';
...
export class LocationService {
   ...
   */
  private _placeResultToGeocoderResult(
  ...
  onPlaceChanged(type: LocationType): () => void {
    return () => {
      const autocomplete =
        type === 'start' ? this.services.autocompleteStart : this.services.autocompleteEnd;
  ...
  private _geocodeAndSetLocation(
    ...
    logger.debug(
      `Geocoding request (${type}, parsed as ${parsedType}, exact: ${!!exactLocation}):`,
      request
    );
    this.showTemporaryMessage(
      this.translate('messageProcessingLocation', {
        parsedType,
      }),
      false
    );
  ...
  private _handleParsedLocationResult(
    ...
    if (!isPlaceInTurkey(placeForTurkeyCheck)) {
      this.showTemporaryMessage(
        this.translate('messageNotInTurkey', {
          parsedType,
        }),
        true
      );
    ...
    this.showTemporaryMessage(
      this.translate('messageSetLocation', {
        type,
        parsedType,
      }),
      false
  ...
  private async _resolveShortUrl(shortUrl: string, type: LocationType): Promise<void> {
    const proxyUrl = SHORT_URL_RESOLVER_PROXY_TEMPLATE.replace(
      '{encoded_url}',
    ...
    try {
      ...
      if (resolvedUrl?.trim() && resolvedUrl.startsWith(GOOGLE_MAPS_URL_PREFIX)) {
        ...
        throw new Error(
          `Proxy returned invalid or empty response: ${JSON.stringify(responseData)}`
        );
      }
    } catch (error) {
  ...
  private _parseCoordinates(
    text: string
  ): { request: google.maps.GeocoderRequest; exactLocation: ExactLocation } | null {
   ...
   */
  private _processPastedLocation(
    text: string,
    type: LocationType,
    ...
    if (coordResult) {
      this._geocodeAndSetLocation(
        type,
        coordResult.request,
        PARSED_TYPE_COORDINATES,
        coordResult.exactLocation
      );
      return;
    }
    ...
    try {
      ...
      logger.debug(
        'Could not parse as URL or error during URL processing, continuing checks:',
        error.message
      );
    }
