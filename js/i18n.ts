import { LANGUAGE_STORAGE_KEY, translations } from './config.js';
...
export function loadLanguage(): void {
  ...
  if (savedLang && isLanguageSupported(savedLang)) {
    ...
    const browserLang = navigator.language
      ? extractLanguageCode(navigator.language)
      : DEFAULT_LANGUAGE;
    ...
    if (isLanguageSupported(browserLang)) {
      ...
      logger.info(
        `Browser language (${browserLang}) not supported or undetectable, defaulting to: ${currentLang}`
      );
    }
  }
...
function handlePluralization(text: string, key: string, params: TranslationParams): string {
  ...
  return text.replace('{count} station(s)', singular).replace('{count} station', singular);
}
    ...
    translationsToApply.forEach(({ attr, key }) => {
      ...
      try {
        if (attr === 'textContent') {
          ...
          if (
            element.classList.contains('shortcut-hint-x') ||
            element.classList.contains('esc-hint')
          ) {
            element.innerHTML = translatedText;
          } else if (element.id === 'loading-text') {
            // Target specific loading text span
        ...
        console.error(
          `Error applying translation key "${key}" (attr: ${attr}) to element:`,
          element,
          e
        );
      }
...
export function handleDocumentClickForDropdown(event: MouseEvent): void {
  ...
  if (
    !languageControlContainer?.contains(event.target as Node) &&
