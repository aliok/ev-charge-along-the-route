...
let filterInputs: {
  ...
  connectorType: document.querySelectorAll(
    '#filter-connector-type input[name="connectorType"]'
  ) as NodeListOf<HTMLInputElement>,
  powerLevel: document.querySelectorAll(
    '#filter-power input[name="powerLevel"]'
  ) as NodeListOf<HTMLInputElement>,
  serviceType: document.querySelectorAll(
    '#filter-service-type input[name="serviceType"]'
  ) as NodeListOf<HTMLInputElement>,
};
...
export function getUIElements(): void {
  ...
  useLocationStartBtn = document.getElementById(
    'use-location-start-btn'
  ...
  mapTypeButtons =
    (mapTypeControlContainer?.querySelectorAll('button') as NodeListOf<HTMLButtonElement>) || [];
  ...
  filterInputs.connectorType = document.querySelectorAll(
    '#filter-connector-type input[name="connectorType"]'
  ) as NodeListOf<HTMLInputElement>;
  filterInputs.powerLevel = document.querySelectorAll(
    '#filter-power input[name="powerLevel"]'
  ) as NodeListOf<HTMLInputElement>;
  filterInputs.serviceType = document.querySelectorAll(
    '#filter-service-type input[name="serviceType"]'
  ) as NodeListOf<HTMLInputElement>;
  ...
  togglePoiVisibilityBtn = document.getElementById(
    'toggle-poi-visibility-btn'
  ) as HTMLButtonElement | null;
...
// --- UI Update Functions ---
export function updateRouteBuilderUI(
...
export function updatePoiVisibilityButtonUI(): void {
  ...
  if (hasMinimumWaypoints) {
    ...
    const iconSvg = state.arePoisVisible
      ? EYE_SLASH_ICON_SVG_ROUTE_PANEL
      : EYE_ICON_SVG_ROUTE_PANEL;
...
export function updateMapTypeButtons(currentMapTypeId: string | google.maps.MapTypeId): void {
  ...
  const mapTypeIdStr =
    typeof currentMapTypeId === 'string' ? currentMapTypeId : String(currentMapTypeId);
...
export function updateBrandFilterModeButton(): void {
  ...
  brandFilterModeButton.setAttribute(
    'title',
    translate(isFavoritesOnly ? 'tooltipBrandFilterFavoritesOnly' : 'tooltipBrandFilterAll')
  );
  brandFilterModeButton.setAttribute(
    'aria-label',
    translate(isFavoritesOnly ? 'tooltipBrandFilterFavoritesOnly' : 'tooltipBrandFilterAll')
...
export function updateOffsetButtonStates(): void {
  ...
  setButtonDisabled(
    offsetStartIncBtn,
    state.startOffsetKm + OFFSET_INCREMENT_KM + state.endOffsetKm >= originalRouteKm
  );
  setButtonDisabled(
    offsetDestIncBtn,
    state.startOffsetKm + state.endOffsetKm + OFFSET_INCREMENT_KM >= originalRouteKm
  );
}
...
export function updateFilterIndicator(): void {
  ...
  const filtersAreActive =
    ...
    JSON.stringify([...state.currentFilters.powerLevels].sort()) !==
      JSON.stringify([...defaultFilters.powerLevels].sort()) ||
    JSON.stringify([...state.currentFilters.serviceTypes].sort()) !==
      JSON.stringify([...defaultFilters.serviceTypes].sort()) ||
...
let handleBrandPreferenceChangeCallback: ((brandName: string, action: BrandAction) => void) | null =
  null;
