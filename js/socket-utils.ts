 ...
 */
export interface SocketAnalysis {
    hasAC: boolean;
    hasDC: boolean;
    maxPower: number;
...
export function hasMatchingPowerLevel(
    ...
    return sockets.some(socket => {
        ...
        const matchesMedium = powerLevels.includes('medium') && 
            power >= POWER_THRESHOLDS.LOW_MAX && power < POWER_THRESHOLDS.MEDIUM_MAX;
...
): { statusClass: string; statusKey: string } {
    if (!availability) {
