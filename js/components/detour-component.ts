...
export class DetourComponent {
    ...
    try {
      const result = await withRetry(
        async () => {
          ...
          if (
            resAtoP.status !== okStatus ||
            resPtoB.status !== okStatus ||
            !resAtoP.result?.routes?.[0]?.legs?.[0] ||
            !resPtoB.result?.routes?.[0]?.legs?.[0]
          ) {
            throw new Error(
              `Detour calc failed. Statuses: A->P: ${resAtoP.status}, P->B: ${resPtoB.status}`
            );
          }
  ...
  triggerPrecalculation(
    updateMarkerDetourText: (_marker: ExtendedMarker, _data: DetourData | null) => void,
    fetchPoiDetails: (_marker: ExtendedMarker, _stationData: any, _silent: boolean) => void
  ): void {
    if (
      !this.routeComponent.isRouteActive ||
    ...
    logger.debug(
      `Pre-processing check. Visible POIs: ${visibleCount}. Limit: ${MAX_PRECALCULATE_DETOURS}. Enabled: ${markersWithinLimit}`
    );
    ...
    if (markersWithinLimit) {
      ...
      logger.debug(
        `Initiated socket pre-fetch for ${socketsToFetchCount}, detour pre-calc for ${detoursToCalcCount} POIs.`
      );
    } else {
