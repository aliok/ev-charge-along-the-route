...
export class MapComponent {
     ...
     */
    initialize(center: { lat: number; lng: number } = defaultCenter, zoom: number = defaultZoom): void {
    ...
    initializeAutocomplete(
        ...
        onPlaceChanged: (type: 'start' | 'end') => () => void
    ): void {
    ...
    setMapType(mapTypeId: string): void {
        if (!this.map) return;
        const mapType = (google.maps.MapTypeId as any)[mapTypeId.toUpperCase()] || google.maps.MapTypeId.ROADMAP;
     ...
     */
    addClickListener(handler: (event: google.maps.MapMouseEvent) => void): void {
        if (!this.map) return;
