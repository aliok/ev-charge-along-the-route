...
function isLatLngInTurkeyBox(lat: number, lng: number): boolean {
    ...
    return lat >= TURKEY_MIN_LAT && lat <= TURKEY_MAX_LAT && lng >= TURKEY_MIN_LNG && lng <= TURKEY_MAX_LNG;
}
 ...
 */
type PlaceLike = google.maps.places.PlaceResult | {
 ...
 */
function extractLatLng(location: google.maps.LatLng | google.maps.LatLngLiteral): { lat: number; lng: number } | null {
...
export function isPlaceInTurkey(place: PlaceLike): boolean {
    ...
    if (place.address_components) {
        const isInTurkey = place.address_components.some(component =>
            component.types.includes('country') && component.short_name === 'TR'
        );
